...
router.get('/receipt/:id/:rid/pdf', authenticateUser, async (req, res) => {
    ...
    try {
        const invoiceData = await fetchInvoiceData(id, companyId, rid);
            ...
            footerTemplate: `
                ...
                <div style='margin: 0 15mm; width: 100%; display: flex; justify-content: space-between; align-items: center; border-top: .5px solid #ddd;'>
                    <div style="font-size: 11px; padding-top: 15px; ">
                        ...
                        ${invoiceData.invoice.status !== 'final' ? 
        ...
        res.setHeader('Content-Disposition', `attachment; filename="receipt_${rid}.pdf"`);
...
async function generateHTML(invoiceData) {
    const { invoice, items, companyDetails, docCustomizations, receipt, cancellationReason, bankAccounts, tax,
        totalWithTax,
        discount,
        withoutTax,
        ...
        return `
            ...
            >
                ...
                <div style="flex-grow: 1; position: relative; z-index: 5;">
                    ...
                    <div style="display: flex; justify-content: space-between; margin-bottom: 25px;">
                        ...
                        <div style="max-width: 45%;">
                            ${invoice.status !== 'draft' ? 
                                `<p style="margin: 0;"><strong style="font-size: 13px; font-weight: bold;">${'Receipt'}: ${receipt.receipt_code}</strong></p>` : ''}
                        ...
                        <div style="width: 20%;">
                            ...
                            <div style="margin-bottom: 2px;">
                                <div style="display: flex; flex-direction: row; gap: 2px; padding: 0; border-radius: 4px;">
                                    <div style="flex: 1; padding: 5px 0;">${formatDate(receipt.created_at)}</div>
                                </div>
                            </div>
                        </div>
                        <div style="width: 25%;">
                            ...
                            <div style="margin-bottom: 2px;">
                                <div style="display: flex; flex-direction: row; gap: 2px; padding: 0; border-radius: 4px;">
                                    <div style="flex: 1; padding: 5px 0;">${formatDate(receipt.document_date)}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 12px; margin-top: 10px;">
                        <thead>
                            <tr style="border-bottom: 1px solid #000;">
                                <th style="text-align: left; padding: 5px 0;">Document</th>
                                <th style="text-align: left; padding: 5px 5px;">Number</th>
                                <th style="text-align: right; padding: 5px 0;">Invoiced</th>
                                <th style="text-align: right; padding: 5px 5px;">Paid</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr style="border-bottom: .5px solid #ddd;">
                                ...
                                <td style="text-align: right; padding: 5px 0;">${setCurrencyFormat(invoice.total_amount)}</td>
                                <td style="text-align: right; padding: 5px 5px;">${setCurrencyFormat(receipt.paid)}</td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    ${cancellationReason ?
                        `<div style="font-size: 11px; margin-top: 25px; width: 50%; margin: 0;">
                            <div style="font-weight: bold; font-size: 12px; display: flex; gap: 2px; border-bottom: 1px solid #000;">
                    ...
                    ${receipt ?
                        `<div style="font-size: 11px; margin-top: 25px; width: 50%; margin: 0;">
                            <div style="font-weight: bold; font-size: 12px; display: flex; gap: 2px; border-bottom: 1px solid #000;">
                                <strong style="flex: 1; padding: 5px 0;">Payment method</strong>
                            </div>
                            
                            <div style="display: flex; flex-direction: column; gap: 2px; padding: 5px 0;">
                                <div style="display: flex; justify-content: space-between; gap: 2px;">
                                    <div style='font-weight: 400;'>${receipt.payment_method}</div>
                                </div>
                            </div>
                        </div>`
                        :``
...
async function fetchInvoiceData(invoiceId, companyId, receiptId) {
    try {
        ...
        const receipt = await new Promise((resolve, reject) => {
            db.query(specificReceiptQuery, [invoiceId, receiptId], (err, result) => {
                if (err) return reject(err);
        ...
        const cancellationReason = await new Promise((resolve, reject) => {
            db.query(cancellationReasonQuery, [receipt.receipt_code, receipt.document_type, companyId], (err, result) => {
                if (err) return reject(err);
                resolve(result[0]); // Expecting a single row for a specific receipt
            });
        });
