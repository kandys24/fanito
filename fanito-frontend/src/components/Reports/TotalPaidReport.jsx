...
const TotalPaidReport = () => {
    ...
    const fetchTaxReport = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${API_URL}/comreport/totalpaid-report`, config);
            setTotalTax(response.data.total_paid);
            setLoading(false);
        } catch (err) {
        ...
        >
            <div className='w-full'>
                <div className='flex justify-between border-b border-black pb-1'>
                    <h1 className="text-3xl">{splitText("Payments Made")}</h1>
                </div>
                <div className="mt-5">
                    <div className="flex flex-col gap-4">
                        ...
                        {totalTax !== null && !loading && !error && (
                            <div className="mt-4 p-4 bg-green-100 dark:bg-green-900 rounded-md text-center">
                                <h3 className="text-lg font-semibold text-green-800 dark:text-green-400">Total Payments Made:</h3>
...
export default TotalPaidReport;